...
#define STATUS_TERMINADO_KERNEL -1		/* valor colocado na variavel status quando uma thread é terminada pelo kernel */
...
/*
 * Representa um arquivo aberto por uma chamada open().
 * Uma thread possui uma lista de struct arquivoAberto.
 */
...
struct thread
{
    ...
    struct file *executavel ;			/* file* do executável do processo, para negar escritas enquanto o processo está executando */
    //void *alocacoes ;
#endif
...
bool thread_compararPrioridadeMaior(const struct list_elem *a, const struct list_elem *b, void *aux UNUSED) ;
bool thread_compararPrioridadeMaiorIgual(const struct list_elem *a, const struct list_elem *b, void *aux UNUSED) ;
void thread_checarPreempt(struct thread *newThread, struct thread *current) ;
